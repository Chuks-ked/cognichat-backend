...
def createGemChatTitle(user_message):
    try:
        model = genai.GenerativeModel(settings.GEMINI_MODEL)
        prompt = (
            "Give a short, descriptive title (max 5 words) for this conversation:\n"
            + user_message
        )
...
def prompt_gemini(request):
    ...
    chat.title = createGemChatTitle(content)
    chat.save()
    ...
    ChatMessage.objects.create(role="assistant", content=gemini_reply, chat=chat)
...
def prompt_gpt(request):
    ...
    if not any(message["role"] == "assistant" for message in openai_messages):
        openai_messages.insert(
            0, {"role": "assistant", "content": "You are a helpful assistant."})
    ...
    ChatMessage.objects.create(
        role="assistant", content=openai_reply, chat=chat)
...
def todays_chat(request):
    chats = Chat.objects.filter(
        created_at__date=today).order_by("-created_at")[:10]
...
def yesterdays_chat(request):
    chats = Chat.objects.filter(
        created_at__date=yesterday).order_by("-created_at")[:10]
    serializer = ChatSerializer(chats, many=True)
    return Response(serializer.data)
...
def seven_days_chat(request):
    chats = Chat.objects.filter(
        created_at__lt=yesterday, created_at__gte=seven_days_ago).order_by("-created_at")[:10]
    serializer = ChatSerializer(chats, many=True)
    return Response(serializer.data)
